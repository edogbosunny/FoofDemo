  ...
  static signUpCtr(req, res) {
    ...
    (async () => {
      try {
        ...
        const resp = await db.query(query, [email, hashpassword, username, userRole]);
        const userId = resp.rows[0].user_id;
        const token = jwt.sign({ id: userId }, config.tokenSecret, { expiresIn: 86400 });
        res.status(200).json({ message: 'Signup successfully', auth: true, token });
      } catch (e) {
        throw e;
      }
    })().catch((e) => {
      console.error(e);
      res.status(500).json({ auth: false, token: null, error: 'The Server encountered a problem' });
    });
  }
  // delete user
